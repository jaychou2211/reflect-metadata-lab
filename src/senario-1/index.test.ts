...
describe('MainDto Validation Scenarios', () => {
        ...
        it('should pass with valid data for all providers', async () => {
            const payload = {
                ...
                providerB: {
                    ...
                    currency: 'TWD',
                    payload: [{ paymentMethod: 'COD', password: 'p2', count: 1 }]
                },
                providerC: {
                    orderId: 'order-xyz',
    ...
    describe('Edge Cases & Invalid Data Scenarios', () => {
        ...
        it('should fail if a nested payload property has incorrect type', async () => {
            const payload = {
                providerC: {
                    orderId: 'order-123',
                    orderNote: 'note',
                    payload: [{ id: 'prod-c', password: 999 }] // Invalid type
                }
            };
            await (await expectValidation(payload)).toFailWith([
        ...
        it('should fail if providerB payload has count less than 1', async () => {
            const payload = {
                providerB: {
                    store: '123e4567-e89b-12d3-a456-426614174000',
                    currency: 'HKD',
                    payload: [{ paymentMethod: 'CREDIT CARD', password: 'p', count: 0 }] // Invalid count
                }
            };
            await (await expectValidation(payload)).toFailWith([
        ...
        it('should fail with multiple errors from multiple providers', async () => {
            const payload = {
                ...
                providerB: {
                    store: '123e4567-e89b-12d3-a456-426614174000',
                    currency: 'TWD',
                    payload: [{ paymentMethod: 'DEBIT', password: 'p', count: 1 }] // Invalid paymentMethod
                }
            };
            await (await expectValidation(payload)).toFailWith([
                'shop must be a string',
