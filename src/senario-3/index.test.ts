...
const expectValidation = async (payload: any) => {
    const instance = plainToInstance(MainDto, payload);
...
describe('MainDto Validation Scenarios (Custom Password Validator)', () => {
        ...
        it('should pass with valid data for all providers', async () => {
            const payload = {
                ...
                providerB: {
                    ...
                    currency: 'TWD',
                    payload: [{ id: 'b1', password: 'P@ssw0rd1', paymentMethod: 'COD', count: 1 }] // Valid strong password
                },
                providerC: {
                    orderId: 'order-xyz',
    ...
    describe('Edge Cases & Invalid Data Scenarios', () => {
        ...
        it('should fail if providerB has invalid UUID', async () => {
            const payload = {
                providerB: {
                    ...
                    currency: 'TWD',
                    payload: [{ id: 'b1', password: 'P@ssw0rd1' }]
                }
            };
            await (await expectValidation(payload)).toFailWith([
        ...
        it('should fail if providerB payload has count less than 1', async () => {
            const payload = {
                providerB: {
                    store: '123e4567-e89b-12d3-a456-426614174000',
                    currency: 'HKD',
                    payload: [{ id: 'b1', password: 'P@ssw0rd1', paymentMethod: 'CREDIT CARD', count: 0 }] // Invalid count
                }
            };
            await (await expectValidation(payload)).toFailWith([
        ...
        it('should fail with multiple errors from multiple providers', async () => {
            const payload = {
                ...
                providerB: {
                    store: '123e4567-e89b-12d3-a456-426614174000',
                    currency: 'TWD',
                    payload: [{ id: 'b1', password: 'weak', paymentMethod: 'DEBIT', count: 1 }] // Invalid password, Invalid paymentMethod
                }
            };
            await (await expectValidation(payload)).toFailWith([
                'shop must be a string',
        ...
        // New test cases for strong password validation
        it('should fail if password in providerB is too simple (missing uppercase)', async () => {
            const payload = {
                providerB: {
                    store: '123e4567-e89b-12d3-a456-426614174000',
                    currency: 'TWD',
                    payload: [{ id: 'b1', password: 'p@ssw0rd1', paymentMethod: 'COD', count: 1 }]
                }
            };
            await (await expectValidation(payload)).toFailWith([
                'Password must contain at least one uppercase letter, one lowercase letter, one digit, and one special character (!@#$%^&*).'
        ...
        it('should fail if password in providerB is too simple (missing special char)', async () => {
            const payload = {
                providerB: {
                    store: '123e4567-e89b-12d3-a456-426614174000',
                    currency: 'TWD',
                    payload: [{ id: 'b1', password: 'Passw0rd1', paymentMethod: 'COD', count: 1 }]
                }
            };
            await (await expectValidation(payload)).toFailWith([
                'Password must contain at least one uppercase letter, one lowercase letter, one digit, and one special character (!@#$%^&*).'
